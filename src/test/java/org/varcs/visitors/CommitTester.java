...
public class CommitTester implements TreeVisitor<Void, CommitNode> {
	...
	private VisitorResult test(CommitNode curCommit, Map<Object, Boolean> assignment)
		throws IOException {
		fileMap = new FileMap();
			...
			while (treeWalk.next()) {
				...
				if (bytes == null) {
					if ((binaryFileNode != null) && binaryFileNode.isActive(assignment, nodeDictionary)) {
						...
						Logger.logInfo(nodeDictionary.getCondition(binaryFileNode.getCondition()).toString());
						Logger.logInfo("");
						Assert.fail();
						continue;
					}
					if ((textFileNode != null) && textFileNode.isActive(assignment, nodeDictionary)) {
						...
						Logger.logInfo(assignment.toString());
						Logger.logInfo(pathString);
						Logger.logInfo(nodeDictionary.getCondition(textFileNode.getCondition()).toString());
						Logger.logInfo("");
						Assert.fail();
						continue;
					}
					...
					if (binary) {
						...
						final List<String> lines1 = textFileNode.getActiveData(assignment, nodeDictionary) //
...
//						}
		...
		for (final Entry<String, BinaryFileNode> entry : fileMap.getBinaryFileMap().entrySet()) {
	...
	private boolean comparePCs(Map<Object, Boolean> assignment, final String pathString,
		final TextFileNode textFileNode, List<LineNode> lineNodeList, final List<String> lines2) {
		if (pathString.matches(AnnotationExtractor.CFileRegex)) {
			lineNodeList = textFileNode.getActiveData(assignment, nodeDictionary).collect(Collectors
				.toList());
			final List<Node> pcList2 = AnnotationExtractor.extractPresenceConditions(
				lines2);
			if (pcList2 != null) {
				int lineNumber = 0;
	...
	private boolean compareFile(List<String> lines1, List<String> lines2, List<LineNode> lineNodeList,
		Map<Object, Boolean> assignment, String pathString) {
