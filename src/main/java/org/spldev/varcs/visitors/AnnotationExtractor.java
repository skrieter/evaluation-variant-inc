...
public class AnnotationExtractor implements TreeVisitor<Void, CommitNode> {
	...
	public void extractAnnotations(CommitNode curCommit, Map<Object, Boolean> assignment) {
		...
		final Literal commitLiteral = new Literal(gitUtils.getVariable(curCommit).orElseThrow(
		...
		values.parallelStream()
			...
			.forEach(fileNode -> {
				final List<LineNode> lineNodes = fileNode.getActiveData(assignment, conditionDictionary).collect(
					Collectors
						.toList());
				final List<String> lines = lineNodes.stream().map(DataNode::getData).collect(Collectors
					.toList());
							...
							if (ppConditions == null) {
								...
								if (lastCondition.getChildren()[0] == chachedPCNode) {
									if (cachedLastCondition != lastCondition) {
										...
										final Node[] newChildren = Arrays.copyOf(lastCondition.getChildren(), oldLength
											+ 1);
